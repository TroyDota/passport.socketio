...
function authorize(options) {
  ...
  if (!auth.passport) {
    try {
      ...
      throw new Error(
        "passport is required to use require('passport'), please install passport"
      );
    }
  }
  ...
  return function (data, accept) {
    ...
    auth.store.get(data.sessionID, function (err, session) {
      let passportSession = session[auth.passport._key];
      if (err)
        return auth.fail(
          data,
      ...
      var userKey;
      if (typeof auth.sessionTags === "string")
        userKey = passportSession[auth.sessionTags];
      else {
        for (var i = 0; i < auth.sessionTags.length && !userKey; i++)
          userKey = passportSession[auth.sessionTags[i]];
      }
      if (!userKey)
        return auth.fail(
          data,
